  ...
  <div class="detail__buttons">
    ...
    <el-dropdown @command="handleMore">
      ...
      <el-dropdown-menu slot="dropdown">
        <template v-if="info && !isNvr">
          <template v-if="inProtocol === 'ehome'">
            <div v-for="num in info.multiStreamSize" :key="num">
              <el-dropdown-item v-if="!isVgroup && getStreamStatus(info.deviceStreams, num) === 'on' && checkPermission(['AdminDevice'])" :command="{type: 'stopDevice', num}">{{ `停用${autoStreamNumObj[num]}` }}</el-dropdown-item>
              <el-dropdown-item v-else-if="!isVgroup && checkPermission(['AdminDevice'])" :command="{type: 'startDevice', num}">{{ `启用${autoStreamNumObj[num]}` }}</el-dropdown-item>
            </div>
          </template>
          <template v-else>
        ...
        <el-dropdown-item v-if="!isAutoCreated && checkPermission(['AdminDevice']) && !isVgroup" :command="{type: 'delete'}">删除</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
