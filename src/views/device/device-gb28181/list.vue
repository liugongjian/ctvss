  ...
  <div class="device-list__container">
    ...
    <div class="filter-container clearfix">
      <div class="filter-container__left">
        <el-button v-if="!isVGroup && (isDir || isManulNVR) && checkPermission(['*'])" key="dir-button" type="primary" @click="goToCreate">{{ isNVR ? '添加子设备' : '添加设备' }}</el-button>
        ...
        <el-button v-if="!isVGroup && isPlatform && checkPermission(['*'])" key="edit-platform" @click="goToUpdate(deviceInfo)">编辑Platform</el-button>
        ...
        <el-dropdown v-if="!isVGroup && checkPermission(['*'])" key="dropdown" placement="bottom" @command="handleBatch">
            ...
            <el-dropdown @command="handleMore">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <template v-if="scope.row.deviceType === 'ipc'">
                  <el-dropdown-item v-if="!isVGroup && scope.row.streamStatus === 'on' && checkPermission(['*'])" :command="{type: 'stopDevice', device: scope.row}">停用流</el-dropdown-item>
                  <el-dropdown-item v-else-if="!isVGroup && checkPermission(['*'])" :command="{type: 'startDevice', device: scope.row}">启用流</el-dropdown-item>
                ...
                <el-dropdown-item v-if="!isVGroup && checkPermission(['*'])" :command="{type: 'update', device: scope.row}">编辑</el-dropdown-item>
                <!--自动创建的子通道不允许删除-->
