  ...
  <div class="app-container">
    ...
    <el-card>
      <el-form
        ...
        label-width="150px"
      >
        <el-form-item label="外部账户ID" prop="extendUserId">
          <el-input v-model="form.extendUserId" placeholder="请输入外部账户ID" />
        </el-form-item>
...
import { getPolicyList, iamCreateRole, iamModifyRole, iamGetRole } from '@/api/accessManage'
@Component({
  name: 'CreateRole'
})
export default class extends Vue {
  ...
  private getPolicyDocument() {
    ...
    for (var node of dirTree.getCheckedNodes()) {
      ...
      resources.push(`ctyun:vss:::${node.inProtocol}-${node.resourceType}:${node.path}`)
    }
    if (resources.length) {
      let policy: any = {
      ...
      policy.statement[0].resource.length > 0 && (this.form.policyDocument = JSON.stringify(policy))
    }
