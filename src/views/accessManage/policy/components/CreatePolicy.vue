...
import { getPolicyInfo, createPolicy, editPolicy } from '@/api/accessManage'
      ...
      if (this.form.actionList[0] === 'vss:*') {
        ...
        this.$nextTick(() => {
          ...
          this.form.actionList.forEach((action: any) => {
            const row = this.systemActionList.find((systemAction: any) => systemAction.actionValue === action)
            if (row) {
    ...
    form.validate(async(valid: boolean) => {
      try {
        if (valid) {
              ...
              'Statement': [
                {
                  ...
                  'Resource': this.resourceType === 'all' ? ['*'] : this.form.resourceList.map((resource: any) => {
          ...
          this.$message.success(`${this.form.policyId ? '编辑策略成功！' : '创建策略成功！'}`)
          this.back()
        } else {
          return false
        }
