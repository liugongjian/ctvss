    ...
    >
      ...
      <el-table-column label="操作" prop="action" class-name="col-action">
        <template slot-scope="scope">
          ...
          <el-button v-if="checkPermission(['ivs:AdminApp'])" type="text" @click.stop="startOrStop(scope.row)">
            {{ parseInt(scope.row.status) ? '停用' : '启用' }}
          </el-button>
          <el-button v-if="checkPermission(['ivs:AdminApp'])" type="text" :disabled="scope.row.status === '1' ? true : false" @click.stop="detach(scope.row)">
            解绑
          </el-button>
        </template>
      </el-table-column>
