<template>
  <div class="detail__buttons">
    <el-button v-if="!isSearch" size="small"><svg-icon name="superior" /> 返回上级</el-button>
    <el-button size="small"><svg-icon name="list" /> 查看通道</el-button>
    <el-button v-if="!isVgroup && checkPermission(['AdminDevice'])" size="small">配置资源包</el-button>
    <el-button v-if="!isVgroup && checkPermission(['AdminDevice'])" size="small"><svg-icon name="edit" /> 编辑</el-button>
    <el-dropdown v-if="!isVgroup" @command="handleMore">
      ...
      <el-dropdown-menu slot="dropdown">
        ...
        <el-dropdown-item :command="{type: 'move'}">移动至</el-dropdown-item>
        <el-dropdown-item v-if="!isAutoCreated && checkPermission(['AdminDevice'])" :command="{type: 'delete'}">删除</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    <el-button size="small"><svg-icon name="refresh" /></el-button>
  </div>
</template>
